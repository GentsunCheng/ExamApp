...
def ensure_db():
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('SELECT pool FROM questions LIMIT 1')
    except sqlite3.OperationalError:
        conn = sqlite3.connect(DB_PATH)
        conn.execute('ALTER TABLE questions ADD COLUMN pool TEXT')
        conn.commit()
        conn.close()
    except Exception:
        try:
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('SELECT random_pick_count FROM exams LIMIT 1')
    except sqlite3.OperationalError:
        conn = sqlite3.connect(DB_PATH)
        conn.execute('ALTER TABLE exams ADD COLUMN random_pick_count INTEGER')
        conn.commit()
        conn.close()
    except Exception:
        try:
